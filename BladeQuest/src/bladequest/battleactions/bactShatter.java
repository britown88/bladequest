...
public class bactShatter extends DelegatingAction {
	...
	BattleAnim getShatterAnimation(PlayerCharacter target)
	{
		...
		state = new BattleAnimObjState(finalFrames * frameLength + 2000, PosTypes.Screen); 
		state.size = new Point(r.width(), r.height());
		state.pos1 = new Point(r.centerX(), r.centerY());
		state.argb(255, 255, 255, 255);
		state.setBmpSrcRect(iceCubeRect.left, iceCubeRect.top, iceCubeRect.right, iceCubeRect.bottom);
		...
		for (int i = 0; i < shatterPoints; ++i)
		{
			...
			p.y = r.top + (int)radius + Global.rand.nextInt((int)(r.height()-radius));
			pts.add(p);
		}
	...
	protected void buildEvents(BattleEventBuilder builder) {
		...
		if (target.hasStatus("frozen"))
		{
			...
			builder.addEventObject(new bactRunAnimation(getSnowImplosion(target)).addDependency(builder.getLast()));
		}
		else
		{
			//aw, miss.
